...
abstract class KotlinCommonBlock(
    ...
    private val ASTNode.wrapForFirstCallInChainIsAllowed: Boolean
        get() {
            ...
            return if (settings.kotlinCommonSettings.WRAP_FIRST_METHOD_IN_CALL_CHAIN)
                unwrapQualifier(down = false)?.isCall == true
            else
                receiverIsCall()
        }
...
private tailrec fun ASTNode.unwrapQualifier(down: Boolean = true): ASTNode? {
    ...
    return if (down)
        psi.baseExpression?.node?.unwrapQualifier(down)
    else
        treeParent.unwrapQualifier(down)
}
